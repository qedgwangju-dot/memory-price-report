...
이미지 요약판은 가장 엄격한 출력물이자 전년 대비 추세 확인용 화면으로 처리합니다. 이미지 안에는 `확인` 상태의 현재 직접 조회값과 현재 접근 가능한 전년 직접 조회값이 모두 있을 때만 전년 대비 숫자, 추세선, 판정 계산을 사용합니다. 전년값은 DRAMeXchange 공개표/공개 JSON의 Internet Archive 캡처, Danawa 24개월 가격추이 API, 또는 저장소에 쌓인 전년 동일일 검증 스냅샷 순서로 확인합니다. 전년 직접치가 없거나 로그인/유료/접근제한/지연 데이터/품목 불일치 등으로 과거 시계열을 직접 확인하지 못하면 임의 추세선을 만들지 않고 점선과 `전년 직접치 부족`으로 표시합니다. `확인 불가` 또는 `지연/불일치 있음` 상태인 값은 원본 객체에 숫자가 남아 있어도 이미지에서 숫자나 추세선으로 노출하지 않고 보류로 표시합니다.

...
이미지 요약판은 가장 엄격한 출력물이자 추세 확인용 화면으로 처리합니다. 이미지 안에는 `확인` 상태의 직접 조회값만 숫자, 변화율, 추세 방향, 판정 계산에 사용합니다. `확인 불가` 또는 `지연/불일치 있음` 상태인 값은 원본 객체에 숫자가 남아 있어도 이미지에서 숫자나 추세 방향으로 노출하지 않고 보류로 표시합니다. 출처·상태를 붙일 수 없는 하단 요약칩에는 숫자를 넣지 않습니다.

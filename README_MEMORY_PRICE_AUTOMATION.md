...
이미지 요약판은 가장 엄격한 출력물이자 추세 확인용 화면으로 처리합니다. 이미지 안에는 `확인` 상태의 직접 조회값만 숫자, 변화율, 추세선, 판정 계산에 사용합니다. 공개 소스가 직접 제공한 변화율이 있으면 그 1기간 변화율을 선으로 표시하고, 로그인/유료/접근제한 등으로 과거 시계열을 직접 확인하지 못하면 임의 추세선을 만들지 않고 점선과 `직접치 부족`으로 표시합니다. `확인 불가` 또는 `지연/불일치 있음` 상태인 값은 원본 객체에 숫자가 남아 있어도 이미지에서 숫자나 추세선으로 노출하지 않고 보류로 표시합니다.
